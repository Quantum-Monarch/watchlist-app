...
class Command(BaseCommand):
    help = 'Fetch movies from TMDb and populate Movie(and Film) table'
    ...
    def handle(self, *args, **kwargs):
        ...
        while total_api_calls < max_api_calls:
            ...
            for movie in movies:
                if movie['id'] not in Film.objects.all():
                    continue
                else:
                    ...
                    series, created = Movies.objects.update_or_create(
                        tmdb_id=tmdb_id,
                        defaults={
                            'name': name,
                    ...
                    if created:
                        created_count += 1
                        self.stdout.write(f"Created Movies instance for Film id={movie.pk}")
